// Реле (имитация замка) Логика: HIGH — реле срабатывает (дверь открыта), LOW — реле отпущено (дверь закрыта)
// 2️⃣ Логика работы

// Изначально дверь закрыта, светодиод горит непрерывно.
// Пользователь вводит код на 4 кнопках.
// Если введён правильный код:
// Дверь открывается (реле включено)
// Светодиод мигает
// Через 10 секунд дверь закрывается, светодиод горит непрерывно
// Если код неверный — ничего не меняется.

// Примечания:
// Кнопки подтянуты к питанию через внутренний резистор ESP32.
// Код можно хранить как массив int correct_code[4] = {1,2,3,4};
// Считывание кнопки через gpio_get_level() (0 = нажата, 1 = отпущена).
